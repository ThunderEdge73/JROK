# List of all available prompts:

> Note that each bullet point corresponds to one prompt effect. Any prompt that contains one or more of a bullet point's listed strings as a substring will trigger the prompt's effect.

> Prompts are case insensitive - capitalize them however you want.

* `banana`, `gros`, `cavendish`, `potassium`
* `cloud`, `9`
* `legendary`, `soul`
* `blueprint`, `brainstorm`, `yuri`
* `photo`, `chad`, `yaoi`, `chud`
* `baron`, `mime`, `infinite`, `infinity`
* `fortune`, `luck`, `oops`, `chance`
* `vanilla`, `mod`, `content`
* `perkeo`, `drunk`, `alcohol`
* `steal`, `stole`, `copy`, `plagiarize`
* `cryptid`, `unbalanced`
* `summarize`, `summary`, `quick`, `speed`, `brief`
* `nope`
* `bug`, `glitch`, `exploit`
* `green`, `environment`
* `needle`
* `space`