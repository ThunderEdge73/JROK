...
extern PRECISION vec2 hallucinated;



extern PRECISION number dissolve;
extern PRECISION vec2 seed;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    vec2 pixel_uv = floor(vec2(uv.x * texture_details.z, uv.y * texture_details.w));
    float n = random(pixel_uv, seed);
    float min_n = 0.8;
    if ( n >= min_n ) {
        vec2 f_coords = vec2(texture_coords.x, texture_coords.y);
        float r = (n - min_n) / (1.0 - min_n);
        float h_mod = floor(sin(r * 38915.12) * 2.5 + 0.5);
        float w_mod = floor(cos(r * 14832.97) * 2.5 + 0.5);
        f_coords.x = f_coords.x - h_mod / (image_details.x);
        f_coords.y = f_coords.y - w_mod / (image_details.y);
        tex.rgb = Texel(texture, f_coords).rgb;
    }

    if (hallucinated == 2 * hallucinated) {
        uv = hallucinated;
    }
